...
namespace MyCompany.Projects
{
    public class AsanaAppService : MyCompanyAppServiceBase, IAsanaAppService
    {
        ...
        public async Task<List<KeyValuePair<string, string>>> GetAllTasksInProject(AsanaTaskInput input)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Get, $"projects/{input.ProjectId}/tasks");
            httpClient.BaseAddress = new Uri("https://app.asana.com/api/1.0/"); // to do: get uri from appsettings 
            httpClient.DefaultRequestHeaders.Add("Accept", "application/json");
            httpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {input.Token}");
            ...
            var stringData = await response.Content.ReadAsStringAsync();
            var workspaces = JsonConvert.DeserializeObject<BaseAsana<AsanaProjectTasks>>(stringData);
            var list = new List<KeyValuePair<string, string>>();
